...
def applications(web_form_name)
  ...
  response = agent.post('https://www.spear.land.vic.gov.au/spear/resources/applicationlist/publicSearch',
                        '{"applicationListSearchRequest":{"searchFilters":[],"searchText":null,"myApplications":false,"watchedApplications":false,"searchInitiatedByUserClickEvent":false,"sortField":"SPEAR_REF","sortDirection":"desc","startRow":0},"tab":"ALL"}', { 'Content-Type' => 'application/json' })
...
page = agent.get(url)
form = page.forms.first
council_names = form.field_with(name: 'councilName').options.map(&:text)[1..-1]
